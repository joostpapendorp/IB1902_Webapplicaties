import {START_GAME_COMMAND_TYPE, STEER_COMMAND_TYPE, PAUSE_COMMAND_TYPE} from "./snake_player.js";
...
function Game(player, difficulties, storage, prepareEngineWith, showIntro){
	this.player = player;
	...
	this.startNewGame = function(difficultyIndex){
	...
	this.receiveKeyInput = function(keyCode) {
		...
		switch(this.state)
		{
			case APP_STARTED_STATE:
				switch(command.type){
					case START_GAME_COMMAND_TYPE:
						this.startNewGame(command.target);
						break;
